...
export function useNetworkMonitor({ transport, onPoorNetworkDetected, onGoodNetworkDetected, onMediumNetworkDetected }: UseNetworkMonitorProps) {
        ...
        try {
            ...
            if (!rtt) {
                ...
                stats.forEach((report: any) => {
                    ...
                    if (report.roundTripTime !== undefined || report.currentRoundTripTime !== undefined || (report.type && report.type.includes("rtt"))) {
            ...
            console.log(`Up: ${Math.round(upBps)}bps, Down: ${Math.round(downBps)}bps, RTT: ${rtt ? rtt.toFixed(2) : "N/A"}ms, Jitter: ${jitter ? jitter.toFixed(2) : "N/A"}ms`);
            ...
            if ((rtt && rtt > POOR_NETWORK_RTT) || (downBps && downBps < POOR_NETWORK_DOWNBPS) || jitter > POOR_NETWORK_JITTER) {
                poorNetworkDetected = true;
            }

            // Xác định tình trạng mạng mạnh
            if (rtt && rtt < GOOD_NETWORK_RTT && downBps && downBps > GOOD_NETWORK_DOWNBPS && jitter < 50) {
