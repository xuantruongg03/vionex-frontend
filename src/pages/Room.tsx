...
import {
    isOrganizationRoomToken,
    generateRoomDisplayId,
} from "@/utils/roomSecurity";
...
const Room = () => {
    ...
    const { verifyRoomMutation, isPending: isVerifyRoomPending } =
        useVerifyRoom();
    const { verifyAccess, isLoading: isVerifyingOrgRoom } =
        useVerifyOrgRoomAccess();
    ...
    useEffect(() => {
        // Check if coming from URL params first
        if (urlRoomId) {
            setRoomId(urlRoomId);

            // Check if it's an organization room session token
            if (isOrganizationRoomToken(urlRoomId)) {
                setIsOrgRoom(true);
                setOrgRoomDisplayId(generateRoomDisplayId(urlRoomId));
            }
        } else {
            ...
            if (roomIdFromParams) {
                ...
                if (isOrganizationRoomToken(roomIdFromParams)) {
                    setIsOrgRoom(true);
                    setOrgRoomDisplayId(
                        generateRoomDisplayId(roomIdFromParams)
                    );
                }
            }
        }
            ...
            if (isOrgRoom) {
                ...
                try {
                    ...
                    if (!verifyResponse.data?.success) {
                        toast.error(
                            "You don't have access to this room or the session has expired"
                        );
                        return;
                    }
                } catch (error) {
                    ...
                    toast.error(
                        "Failed to verify room access. Please try again."
                    );
                    return;
                }
                ...
                const finalUserName =
                    user?.name ||
                    userName.trim() ||
                    nameDefault[Math.floor(Math.random() * nameDefault.length)];
                ...
                <div className='max-w-6xl w-full mx-auto'>
                    ...
                    <div className='text-center mb-8 md:mb-12'>
                        ...
                        <p className='text-base mt-4 md:text-lg text-gray-600 dark:text-gray-300 max-w-2xl mx-auto leading-relaxed'>
                            {isOrgRoom
                                ? "Secure organization meeting - Test your camera and audio before joining"
                                : "Test your camera and audio, then enter your meeting details"}
                        </p>
                        {isOrgRoom && orgRoomDisplayId && (
                            <div className='mt-4'>
                                <span className='inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200'>
                                    Room ID: {orgRoomDisplayId}
                                </span>
                            </div>
                        )}
                    </div>{" "}
                    ...
                    <div className='grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-12 items-center max-w-5xl mx-auto'>
                        ...
                        <div className='relative group order-1 lg:order-1'>
                            <VideoPreview
                                userName={user?.name || debouncedUserName}
