...
const GoogleLoginButton = ({ isLogin, isGoogleLoggingIn, onGoogleLogin }: { isLogin: boolean; isGoogleLoggingIn: boolean; onGoogleLogin: (userInfo: any) => void }) => {
    ...
    return (
        <Button type='button' variant='outline' className='w-full h-10 border border-gray-200 dark:border-gray-600 bg-white dark:bg-slate-800 hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors duration-200 font-medium' onClick={initiateGoogleLogin} disabled={isGoogleLoggingIn}>
            <div className='flex items-center justify-center gap-3'>
                ...
                <span className='text-gray-700 dark:text-gray-200'>{isGoogleLoggingIn ? "Signing in..." : `${isLogin ? "Sign in" : "Sign up"} with Google`}</span>
            </div>
        </Button>
    );
};
