...
export const TranslationCabinSidebar: React.FC<
    TranslationCabinSidebarProps
> = ({ isOpen, setIsOpen, roomId, availableUsers, onConsumeTranslation }) => {
    const { loading, error, createCabin } = useTranslationCabin();
    ...
    const {
        cabins,
        loading: listLoading,
        refetch: refetchCabins,
    } = useListCabin(roomId, user?.username || "");
            ...
            <motion.div
                ...
                className={`${
                    isMobile
                        ...
                        : "fixed right-0 top-0 h-screen w-80 bg-white dark:bg-gray-900 border-l border-gray-200 dark:border-gray-700 z-30"
                } flex flex-col translation-cabin-sidebar`}
            >
                {/* Header */}
                ...
                {/* Error Message */}
                {error && (
                    <div className="mx-4 mt-4 p-3 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-lg text-sm">
                        {error}
                    </div>
                )}
                    ...
                    {activeTab === "list" && (
                        <div className="p-4">
                            {listLoading ? (
                                ...
                                <TranslationCabinList
                                    roomId={roomId}
                                    cabins={cabins}
                                    availableUsers={availableUsers}
                                    sourceUserId={user?.username || ""}
                                    onCabinDestroyed={refetchCabins}
                                />
                            )}
                        </div>
                    )}
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div>
                                            ...
                                            <select
                                                ...
                                                className="w-full p-3 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                                            >
                                                {SUPPORTED_LANGUAGES_TRANSLATION.map(
                                                    (lang) => (
                                                        <option
                                                            key={lang.code}
                                                            value={lang.code}
