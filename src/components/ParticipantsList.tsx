            ...
            sfuSocket.emit(
                ...
                (file: any) => {
                    if (file && file.success) {
                        ...
                        a.href = url;
                        a.download = `behavior-logs-${roomId}-${userId}-${new Date().toISOString().slice(0, 10)}.xlsx`;
                        ...
                        toast.success(`Downloaded log file for user ${userId}`);
                    } else if (file && !file.success) {
                        toast.error(file.error || "Failed to download log file");
                    }
                }
            );
        } catch (error) {
        ...
        <Sheet>
            <SheetTrigger asChild>
                <Button variant='outline' size='icon' className='relative'>
                    ...
                    <span className='absolute -top-1 -right-1 bg-blue-500 text-white text-xs rounded-full h-4 w-4 flex items-center justify-center'>{userCount}</span>
                </Button>
            </SheetTrigger>
                ...
                <div className='mt-4 flex flex-col h-[calc(100vh-180px)]'>
                    <div className='overflow-y-auto flex-1 pr-2'>
                        <div className='space-y-2'>
                            {usersList.map((user) => (
                                <div key={user.peerId} className='flex items-center justify-between p-3 rounded-lg hover:bg-secondary'>
                                            ...
                                            {user.userInfo?.avatar ? (
                                                <img
                                                    ...
                                                    onError={(e) => {
                                                        ...
                                                        if (parent) {
                                                            parent.innerHTML = `<div class="w-full h-full flex items-center justify-center text-white text-sm font-semibold">${user.peerId.charAt(0).toUpperCase()}</div>`;
                                                        }
                                                    }}
                                                />
                                            ) : (
                                                <div className='w-full h-full flex items-center justify-center text-white text-sm font-semibold'>{user.peerId.charAt(0).toUpperCase()}</div>
                                            )}
                                        </div>
                                    ...
                                    {!user.isMe && (
                                        <div className='flex gap-2 flex-shrink-0'>
                                            {togglePinUser && (
                                                <Button variant='ghost' size='sm' className={`hover:bg-blue-50 ${user.isPinned ? "text-blue-500 hover:text-blue-700" : "text-gray-500 hover:text-blue-500"}`} onClick={() => handleTogglePin(user.peerId)} title={user.isPinned ? "Unpin user" : "Pin user"}>
                                                    {user.isPinned ? <PinOff className='h-4 w-4' /> : <Pin className='h-4 w-4' />}
                                                </Button>
                                            )}
                                            {isCreator && isMonitorActive && (
                                                <Button variant='ghost' size='sm' className='text-blue-500 hover:text-blue-700 hover:bg-blue-50' onClick={() => handleDownloadUserLog(user.peerId)} title='Download user activity log'>
                                                    <FileSpreadsheet className='h-4 w-4' />
                                                </Button>
                                            )}
                                            {isCreator && (
                                                <Button variant='ghost' size='sm' className='text-destructive hover:text-destructive hover:bg-destructive/10' onClick={() => handleRemoveParticipant(user.peerId)}>
